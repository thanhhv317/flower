  ...
  static start(options) {
    ...
    Module.prototype.require = function require(file) {
      ...
      Object.keys(exp).forEach((key) => {
        ...
        // if (pluginController.Pattern.test(absFilePath)) {
        //   pluginController.Wrapper(exp, Tracer, options, key);
        // }
        if (pluginService.Pattern.test(absFilePath)) {
          let notAccessPath = ['auth.service', 'oauth.service', 'user.service'];
          if (
            !(
              absFilePath.includes(notAccessPath[0]) ||
              absFilePath.includes(notAccessPath[2]) ||
              absFilePath.includes(notAccessPath[3])
            )
          ) {
